(function($) {
    $(document).ready(function() {
        ...
        var tableLoadProductCategoryList = $('#productCategoryDT').DataTable({
            ajax: '/admin/productCategoryListTable',
            columns: [{
                    "data": null,
                    "sortable": false,
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        return [`<img src="/uploads/product/category/${row.category_image}" ' + "${row.category_image}" + 'alt="Admin" class="rounded-circle" width="65px" height="65px"></img>`];
                    }
                },
                {
                    "data": "category_name"
        ...
        $('#addProductCategoryFrom').on('submit', function(event) {
            ...
            $.ajax({
                ...
                error: function(error) {
                    console.log(error);
                    $('#nameError').text(error.responseJSON.errors.category_name);
        ...
        $(document).on('click', 'a#editProductCategoryModal', function(e) {
            ...
            $('#edit_productCategory_modal input[name="productCategoryID"]').val(productCategoryID);

            //alert(leaveRequestID);
            $.ajax({
                url: '/admin/edit-productCategory/' + productCategoryID,
                method: 'get',
                dataType: "json",
                success: function(data) {
                    console.log(data);
                    ...
                    $('#edit_productCategory_modal input[name="category_nameEdit"]').val(data.category_name);
                    $('#edit_productCategory_modal input[name="imageEdit"]').val('');
                    $("#statusEdit_c").val(data.status).change();
        ...
        $('#editProductCategoryFrom').on('submit', function(event) {
            ...
            $('#nameErrorEdit').text('');
            $('#statusErrorEdit').text('');

            let productCategoryID = $('#edit_productCategory_modal input[name="productCategoryID"]').val();
            ...
            $.ajax({
                ...
                error: function(error) {
                    console.log(error);
                    $('#nameErrorEdit').text(error.responseJSON.errors.category_nameEdit);
                    $('#statusErrorEdit').text(error.responseJSON.errors.statusEdit);
                    //console.log(data);
                    //swal("Warning!", "All The Field is required!", "warning");
                    //alert('error');
                    //console.log(response);
        ...
        $(document).on('click', '#deleteProductCategory', function(e) {
            ...
            $.ajax({
                url: '/admin/delete-category/' + productCategoryID,
                method: 'POST',
                dataType: "json",
